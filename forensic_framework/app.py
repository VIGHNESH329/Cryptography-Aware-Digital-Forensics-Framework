...
def login():
    st.markdown("""
        <div style='text-align: center; padding: 20px;'>
            <h1 style='font-size: 3rem;'>🕵️ Forensic Portal</h1>
            <p style='color: #94a3b8;'>Secure Evidence Gateway & Public Archive</p>
        </div>
    """, unsafe_allow_html=True)
    ...
    with col1:
        st.markdown("""
            <div style='background: linear-gradient(135deg, #1e293b 0%, #0f172a 100%); padding: 30px; border-radius: 15px; border: 1px solid #38bdf8;'>
                <h3 style='color: #38bdf8;'>🏛️ Admin Entry</h3>
                <p style='font-size: 0.9rem; color: #94a3b8;'>Master control for lab auditors and administrators.</p>
            </div>
        """, unsafe_allow_html=True)
        with st.form("admin_login"):
            ...
            if st.form_submit_button("Authorize Admin"):
                ...
                user = conn.execute("SELECT * FROM users WHERE username=? AND password=? AND role='Auditor'", (u, p)).fetchone()
                conn.close()
                if user:
    ...
    with col2:
        st.markdown("""
            <div style='background: linear-gradient(135deg, #1e293b 0%, #0f172a 100%); padding: 30px; border-radius: 15px; border: 1px solid #10b981;'>
                <h3 style='color: #10b981;'>🌐 Public Portal</h3>
                <p style='font-size: 0.9rem; color: #94a3b8;'>Open access for evidence submission and case tracking.</p>
            </div>
        """, unsafe_allow_html=True)
        
        st.write("---")
        st.info("No credentials required for Public Access. You can submit evidence and view analysis results.")
        if st.button("Enter Public Portal", use_container_width=True):
            st.session_state.logged_in = True
            st.session_state.username = "Guest Submitter"
            st.session_state.role = "Public"
            # Public sessions use a shared transient key for basic signing/integrity
            priv, _ = generate_rsa_keys()
            st.session_state.private_key = priv
            st.rerun()
...
# Define menu based on roles
if st.session_state.role == "Auditor":
    menu_options = ["Dashboard", "Evidence Intake", "Forensic Analysis", "Tamper Sentinel", "Reports & Export", "Settings"]
else:
    menu_options = ["Dashboard", "Evidence Intake", "Forensic Analysis", "Reports & Export"]

menu = st.sidebar.radio("Modules", menu_options)
...
# --------------------------------------------------
    ...
    with st.form("upload_form"):
        ...
        # If Public, we need their name
        submitter_name = ""
        if st.session_state.role == "Public":
            submitter_name = st.text_input("Your Full Name (Submitter)")
        
        file = st.file_uploader("Upload Evidence (Image, PDF, Log, etc.)")
        submit = st.form_submit_button("Process Evidence")

        if submit and file:
            if st.session_state.role == "Public" and not submitter_name:
                st.error("Please provide your name for the Chain of Custody.")
            else:
                # 1. Save File
                ...
                # 4. Save to DB
                officer = st.session_state.username if st.session_state.role != "Public" else submitter_name
                cursor = conn.cursor()
                cursor.execute(
                    "INSERT INTO evidence (case_id, filename, hash, signature, metadata, investigator, timestamp) VALUES (?,?,?,?,?,?,?)",
                    (case_id, file.name, h, sig, meta, officer, datetime.datetime.now())
                )
                ...
                cursor.execute(
                    ...
                    (evidence_id, "Evidence Uploaded & Signed", officer, datetime.datetime.now())
                )
                conn.commit()
                st.success(f"✅ Evidence Secured and Vaulted. Filed by: {officer}")
                st.json(ai_res)
...
# --------------------------------------------------
    ...
    tab1, tab2 = st.tabs(["Quick Scan", "Manual Verification"])
    
    with tab1:
        st.subheader("System-Wide Integrity Check")
    ...
    with tab2:
        st.subheader("Manual Evidence Re-Verification")
        st.info("Upload a file to compare it against its original vaulted hash. This detects even single-character changes.")
        
        if df.empty:
            st.warning("No records in database to verify against.")
        else:
            selected_file_record = st.selectbox("Select Evidence Record to Verify Against", df['filename'].tolist())
            uploaded_verification_file = st.file_uploader("Upload File to Verify", key="verify_uploader")
            
            if st.button("Verify Integrity Now"):
                if uploaded_verification_file:
                    # Get original hash from DB
                    original_hash = df[df['filename'] == selected_file_record].iloc[0]['hash']
                    
                    # Calculate hash of uploaded file content
                    uploaded_bytes = uploaded_verification_file.getvalue()
                    uploaded_hash = generate_hash(uploaded_bytes)
                    
                    if uploaded_hash == original_hash:
                        st.success(f"✅ VERIFIED: The uploaded file matches the original evidence record for '{selected_file_record}'. No tampering detected.")
                    else:
                        st.error(f"🚨 TAMPERED: The uploaded file DOES NOT match the original record for '{selected_file_record}'!")
                        st.warning(f"Original Hash: `{original_hash}`")
                        st.warning(f"Uploaded Hash: `{uploaded_hash}`")
                else:
                    st.error("Please upload a file to perform verification.")
...
# --------------------------------------------------
    ...
    if not case_ids:
        ...
        if st.button("Generate & Download PDF Report"):
            ...
            pdf_path = generate_pdf_report(case, case_evidence, logs, analyze_evidence_ai)
